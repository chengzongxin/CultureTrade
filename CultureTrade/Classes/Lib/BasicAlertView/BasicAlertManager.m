...
#define AlertViewWidth ScreenSize.width - 20 //350.0f
...
- (void)prepareAlertToDisplay:(BasicAlertView *)entity
{
    ...
    UILabel *_subtitleLable = [[UILabel alloc] initWithFrame:CGRectMake(20, 50, AlertViewWidth, 20)];
    ...
    UILabel *_messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(30, 80, AlertViewWidth, 50)];
...
//                           forState:UIControlStateHighlighted];
        ...
        CGFloat x = (AlertViewWidth - btnImg.size.width * 2)/3;
        _button.frame = CGRectMake(x + i * (btnImg.size.width + x), 150,btnImg.size.width, btnImg.size.height);
        _button.tag = i;
        ...
        if (entity.buttons.count == 1) {
            CGFloat x = (AlertViewWidth - btnImg.size.width)/2;
            CGFloat y = _button.frame.origin.y;
            _button.frame = (CGRect){{x,y},btnImg.size};
        }
