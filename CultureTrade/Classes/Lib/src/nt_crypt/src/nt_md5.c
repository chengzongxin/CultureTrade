...
void Md5HashBuffer(void *pOutBuffer, const void *pInBuffer, uint32_t dwLength)
{
	...
	*/
	...
	MD5Init(md5Info);
	MD5Update(md5Info, (unsigned char*)pInBuffer, dwLength);
	MD5Final((unsigned char*)pOutBuffer, md5Info);
...
static struct {
	...
	uint8_t digest[ MD5_DIGEST_LENGTH ];	/* Digest of data */
	} digestValues[] = {
	{ "", 0,
...
int Md5Test( void )
{
	...
	for( i = 0; digestValues[ i ].data != NULL; i++ )
	{
		...
		Md5HashBuffer( digest, digestValues[ i ].data,
