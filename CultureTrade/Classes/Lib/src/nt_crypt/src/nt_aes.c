...
static void aes_gen_tables( void )
{
    ...
    for( i = 0; i < 256; i++ )
    {
        ...
        RT0[i] = ( (unsigned long) MUL( 0x0B, y )       ) ^
                 ( (unsigned long) MUL( 0x0D, y ) <<  8 ) ^
                 ( (unsigned long) MUL( 0x09, y ) << 16 ) ^
                 ( (unsigned long) MUL( 0x0E, y ) << 24 ) ;
 ...
 */
static unsigned long KT0[256] ;
static unsigned long KT1[256] ;
static unsigned long KT2[256] ;
static unsigned long KT3[256] ;
                      ...
                      int len )
{
    ...
    while( len > 0 )
    {
		...
		aes_encrypt(ctx, st, st) ;
        for (i = 0; i < bytes; ++i){
            output[i] = input[i] ^ st[i] ;
            iv[16 - bytes + i] = output[i] ;
        }
...
{
	int cfb_len = len % 16;
	int cbc_len = len - cfb_len;

	if (cbc_len > 0){
		aes_cbc_encrypt(ctx, iv, input, output, cbc_len) ;
		input += cbc_len;
		output += cbc_len;
	}
	...
	if (cfb_len > 0){
		int len1 = cfb_len % 4;
		int len2 = cfb_len - len1;
		if (len2 > 0){
			aes_cfb_encrypt(ctx, iv, 4, input, output, len2) ;
			input += len2;
			output += len2;
		}
		
		if (len1 > 0){
			aes_cfb_encrypt(ctx, iv, 1, input, output, len1) ;
		}
	}
	return;
}
                   ...
                   int len)
{
	...
	int cbc_len = len - cfb_len;
	if (cbc_len > 0){
		aes_cbc_decrypt(ctx, iv, input, output, cbc_len) ;
		input += cbc_len;
		output += cbc_len;
	}
	...
	if (cfb_len > 0){
		...
		int len2 = cfb_len - len1;
		if (len2 > 0){
			aes_cfb_decrypt(ctx, iv, 4, input, output, len2) ;
			input += len2;
			output += len2;
		}
		
		if (len1 > 0){
			aes_cfb_decrypt(ctx, iv, 1, input, output, len1) ;
		}
	}
	return;
}

static unsigned hash_str(const unsigned char *str, int len)
{
    // 这个hash 函数据说是比较好的(对字符串来说) << 程序设计实践>>
    static unsigned magic = 31;
               ...
               int osize)
{
	...
	// 网络字节序
	h = hash_str(input, ilen) ;
	output[0] = h & 0xFF;
	output[1] = (h >> 8) & 0xFF;
	output[2] = (h >> 16) & 0xFF;
	output[3] = (h >> 24) & 0xFF;

	aes_mix_encrypt(ctx, iv, input, output + 4, ilen) ;
	return 0;
}
               ...
               unsigned char *output)
{
	...
	// 网络字节序还原
	hash1 = input[3] ;
	hash1 = (hash1 << 8) + input[2] ;
	hash1 = (hash1 << 8) + input[1] ;
	hash1 = (hash1 << 8) + input[0] ;

	aes_mix_decrypt(ctx, iv, input + 4, output, ilen - 4) ;

	hash2 = hash_str(output, ilen - 4) ;
	if (hash1 != hash2){
		// 检校值不匹配，解密失败
		return 1;
	}
...
int AesEncrypt(const uint8_t * pbInBuf, uint32_t dwInBufLen, const uint8_t * pbKey, uint32_t dwKeyLen, uint8_t * pbOutBuf, uint32_t * pdwOutBufLen)
{
	...
	if (dwInBufLen < 4 || *pdwOutBufLen + 4 < dwInBufLen) {
		return -1;
	}

	aes_set_key(&ctx, pbKey, dwKeyLen << 3); //算出bit位加密密钥的长度(* 8)
	*pdwOutBufLen = dwInBufLen + 4;

	return aes_my_encrypt(&ctx, pbInBuf, (int)dwInBufLen, pbOutBuf, *pdwOutBufLen);
}
...
int AesDecrypt(const uint8_t * pbInBuf, uint32_t dwInBufLen, const uint8_t * pbKey, uint32_t dwKeyLen, uint8_t * pbOutBuf, uint32_t * pdwOutBufLen)
{
	...
	*pdwOutBufLen = dwInBufLen - 4;
	...
	return aes_my_decrypt(&ctx, pbInBuf, (int)dwInBufLen, pbOutBuf);
}
               ...
               int osize)
{
	...
	MD5Init(&md5ctx) ;
	MD5Update(&md5ctx, input, ilen) ;
	MD5Final(output, &md5ctx) ;

	aes_mix_encrypt(ctx, iv, input, output + 16, ilen) ;
	return 0;
}
               ...
               unsigned char *output)
{
	...
	aes_mix_decrypt(ctx, iv, input + 16, output, ilen - 16) ;

	MD5Init(&md5ctx) ;
	MD5Update(&md5ctx, output, ilen - 16) ;
	MD5Final(md5str, &md5ctx) ;

	if (memcmp(md5str, input, 16) != 0){
		// 检校值不匹配，解密失败
		printf("检校值不匹配，解密失败\n") ;
		return 1;
	}
