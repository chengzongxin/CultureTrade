...
{
    // to do
    return [[BaseSwitch alloc] initWithStringsArray:strings];
}
...
//        label.adjustsLetterSpacingToFitWidth = YES;
        ...
        NSDictionary *attrDict = @{NSKernAttributeName : @(3)
                                   ,NSFontAttributeName : [UIFont systemFontOfSize:10]
                                   };
        label.attributedText = [[NSAttributedString alloc] initWithString:label.text attributes:attrDict];
...
- (void)layoutSubviews
{
    ...
    self.sliderView.frame = CGRectMake(sliderWidth * self.selectedIndex + self.sliderOffset - kBorder / 2, self.backgroundView.frame.origin.y + self.sliderOffset - kBorder / 2, sliderWidth - self.sliderOffset * 2 + kBorder, self.frame.size.height - self.sliderOffset * 2 + kBorder);
